#include "stdafx.h"
#include "SQLitePluginEventLoggerRequester.h"
...
namespace database { namespace sqlite { namespace requesters { 
   ...
   CSQLitePluginEventLoggerRequester::CSQLitePluginEventLoggerRequester(const CSQLiteDataProvider & databaseHandler, boost::shared_ptr<CSQLiteRequester> & databaseRequester)
      :m_databaseHandler(databaseHandler), m_databaseRequester(databaseRequester)
   {
   }

   CSQLitePluginEventLoggerRequester::~CSQLitePluginEventLoggerRequester()
   {
   }

   // IPluginEventLoggerRequester implementation

   int CSQLitePluginEventLoggerRequester::addEvent(const std::string & pluginName, const std::string & pluginVersion, const shared::plugin::information::EReleaseType pluginReleaseType, const database::entities::EEventType eventType, const std::string & message /*= CStringExtension::EmptyString*/)
   {
   ...
   std::vector<boost::shared_ptr<database::entities::CPluginEventLogger> > CSQLitePluginEventLoggerRequester::getPluginEvents(const std::string & pluginName, const std::string & pluginVersion, const shared::plugin::information::EReleaseType pluginReleaseType, const boost::posix_time::ptime & fromDate)
   {
      CQuery qSelect;
      qSelect. Select().
         From(CPluginEventLoggerTable::getTableName()).
