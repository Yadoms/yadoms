...
public:
   ...
   class CCapacity
   {
      std::string label;   // Can be a known capacity ("temperature", "humidity"...) or a specific capacity (must be declare at declareNewDevice call)
      bool canRead;        // Typically a sensor (read-only) like a temperature sensor
      bool canWrite;       // Typically a command (like a lamp, a send of message...)
   };
