...
namespace shared
{
   namespace versioning
   {
      ...
      CSemVer::CSemVer(const std::string& stringVersion) noexcept(false)
      {
         parse(stringVersion);
      }

      CSemVer::CSemVer(int major, int minor, int patch)
         : m_major(major), m_minor(minor), m_patch(patch), m_prerelease(""), m_buildMetadata("")
      {
      }

      CSemVer::CSemVer(int major, int minor, int patch, const std::string& prerelease)
         : m_major(major), m_minor(minor), m_patch(patch), m_prerelease(prerelease), m_buildMetadata("")
      {
      }

      CSemVer::CSemVer(int major, int minor, int patch, const std::string& prerelease, const std::string& buildMetaData)
         : m_major(major), m_minor(minor), m_patch(patch), m_prerelease(prerelease), m_buildMetadata(buildMetaData)
      {
      }

      bool CSemVer::operator<(CSemVer const& rhs) const
      {
      ...
      int CSemVer::getMajor() const
      {
         return m_major;
      }

      int CSemVer::getMinor() const
      {
         return m_minor;
      }

      int CSemVer::getPatch() const
      {
         return m_patch;
      }

      const std::string& CSemVer::getPrerelease() const
      {
         return m_prerelease;
      }

      const std::string& CSemVer::getBuildMetadata() const
      {
         return m_buildMetadata;
      }

      int CSemVer::compare(CSemVer const& rhs) const
      {
         // 0 : equal
         // 1 : this > rhs
         //-1 : this < rhs
      ...
      std::string CSemVer::toString() const
      {
      ...
      void CSemVer::parse(const std::string& version) noexcept(false)
      {
      ...
      std::ostream& operator<<(std::ostream& oss, const CSemVer& rhs)
      {
         oss << rhs.toString();
         return oss;
      }
