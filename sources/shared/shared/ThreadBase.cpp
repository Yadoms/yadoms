...
namespace shared
{
   constexpr int CThreadBase::DefaultStopTimeoutSeconds = 100;


   CThreadBase::CThreadBase(const std::string& threadName,
                            bool bRethrowDoWorkExceptions)
      : m_threadName(threadName),
        m_stopTimeoutSeconds(DefaultStopTimeoutSeconds),
        m_bRethrowDoWorkExceptions(bRethrowDoWorkExceptions),
        m_stopping(false)
   {
      BOOST_ASSERT(!threadName.empty());
   }
   ...
   void CThreadBase::start()
   {
      ...
      //start the thread
      m_thread = boost::make_shared<boost::thread>([this] { doWorkInternal(); });
   }
