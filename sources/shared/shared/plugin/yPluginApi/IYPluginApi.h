   ...
   namespace plugin
   {
      namespace yPluginApi
      {
         ...
         public:
            ...
            //-----------------------------------------------------
            ///\brief Declare new manually created device to Yadoms. Unlike declareDevice, device name will be autogenerated to be unique.
            ///\param    [in]    userDeviceName    The user device name (shown to user)
            ///\param    [in]    type              The device configuration type (ex : "osCN185"). This is the identifiers used to retreive configuration schema package.json (deviceConfiguation.staticConfigurationSchema.schemas.anySchemaName.types.osCN185). Never displayed or modified.
            ///\param    [in]    model             The device model or description (ex : "Oregon Scientific CN185"). Displayed and editable by user
            ///\param    [in]    details           Device details
            ///\return                             The new device name (needed to add keywords...)
            ///\note It's important to understand difference between userDeviceName (shown to user and modifiable by user) and device name (used by the plugin, never modifiable)
            //-----------------------------------------------------
            virtual std::string declareManuallyCreatedDevice(const std::string& userDeviceName,
                                                             const std::string& type,
                                                             const std::string& model,
                                                             const CDataContainer& details = CDataContainer::EmptyContainer) = 0;
