...
namespace shared
{
   constexpr size_t MinUserBufferSize(unsigned int itemcount) { return (12 + (32 * itemcount)); }
   ...
   std::unique_ptr<CDataContainer> CDataContainer::makeUnique()
   {
      return std::make_unique<CDataContainer>();
   }
   ...
   std::unique_ptr<CDataContainer> CDataContainer::makeUnique(unsigned int estimatedDataSize, unsigned int estimatedItemCount)
   {
      return std::make_unique<CDataContainer>(estimatedDataSize, estimatedItemCount);
   }
   ...
   std::unique_ptr<CDataContainer> CDataContainer::makeUnique(const std::string& initialData)
   {
   ...
   std::unique_ptr<CDataContainer> CDataContainer::makeUnique(const std::map<std::string, std::string>& initialData)
   {
      return std::make_unique<CDataContainer>(initialData);
   }
   ...
   std::unique_ptr<CDataContainer> CDataContainer::makeUnique(rapidjson::Value& d)
   {
      return std::make_unique<CDataContainer>(d);
   }
   ...
   std::unique_ptr<CDataContainer> CDataContainer::makeUnique(rapidjson::Document& d)
   {
      return std::make_unique<CDataContainer>(d);
   }

   const boost::shared_ptr<CDataContainer> CDataContainer::EmptyContainerSharedPtr = make();
   const std::unique_ptr<CDataContainer> CDataContainer::EmptyContainerUniquePtr = makeUnique();
   const CDataContainer CDataContainer::EmptyContainer = CDataContainer();

   CDataContainer::CDataContainer()
      : m_tree_allocator_initial_buffer(nullptr),
        m_tree_allocator(nullptr)
   {
      m_tree.SetObject();
   }

   CDataContainer::CDataContainer(unsigned int estimatedDataSize,
                                  unsigned int estimatedItemCount)
      : m_tree_allocator_initial_buffer(operator new(estimatedDataSize + MinUserBufferSize(estimatedItemCount))),
        m_tree_allocator(
           new rapidjson::MemoryPoolAllocator<rapidjson::CrtAllocator>(m_tree_allocator_initial_buffer,
                                                                       estimatedDataSize + MinUserBufferSize(estimatedItemCount))),
   ...
   CDataContainer::CDataContainer(const rapidjson::Value& d)
      : m_tree_allocator_initial_buffer(nullptr),
        m_tree_allocator(nullptr)
   {
      m_tree.SetObject();
   ...
   CDataContainer::~CDataContainer()
   {
      ...
      if (m_tree_allocator_initial_buffer != nullptr)
         operator delete(m_tree_allocator_initial_buffer);
   }

   boost::shared_ptr<CDataContainer> CDataContainer::getChild(const std::string& parameterName,
                                                              char pathChar) const
   {
   ...
   {
      if (&rhs == this)
         throw std::runtime_error("Can not self-assign CDataContainer");
   ...
   void CDataContainer::deserializeFromFile(const std::string& file)
   {
      ...
      rapidjson::FileReadStream bis(fp, (buffer.get()), fileSize);
   ...
   {
      os << "Size=" << getPointer()->GetAllocator().Size() << " ChunkSize=" << getPointer()->GetAllocator().Capacity() << " StackCapcity=" <<
         getPointer()->GetStackCapacity() << std::endl;
   }
   ...
   std::string CDataContainer::generatePath(const std::string& parameterName, const char pathChar)
   {
      ...
      const int c = static_cast<int>(parameterName.size());
   ...
   CDataContainer CDataContainer::find(const std::string& parameterName,
                                       const boost::function<bool(const CDataContainer&)>& whereFct,
   ...
   {
      const rapidjson::Value& v = rapidjson::Pointer(generatePath(parameterName, pathChar).c_str()).Create(m_tree).SetArray();
      return v.IsArray();
   }
   ...
   char CDataContainer::convertToByte(const rapidjson::Value& v)
   {
      ...
      {
         const double b = v.GetDouble();
         if (b < static_cast<double>(INT8_MIN) || b > INT8_MAX)
   ...
   short CDataContainer::convertToShort(const rapidjson::Value& v)
   {
      ...
      {
         const double b = v.GetDouble();
         if (b < static_cast<double>(INT16_MIN) || b > INT16_MAX)
            throw exception::COutOfRange((boost::format("%1% is not assignable to signed Short/Int16") % b).str());
         return static_cast<short>(b);
      }
      if (v.IsFloat())
