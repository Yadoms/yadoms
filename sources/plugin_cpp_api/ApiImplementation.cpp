...
namespace plugin_cpp_api
{
    ...
    void CApiImplementation::send(const plugin_IPC::toYadoms::msg& msg) const
    {
        try
        {
            boost::lock_guard lock(m_sendMutex);

            if (!m_sendMessageQueue || !m_messageCutter)
                throw std::runtime_error(std::string("CApiImplementation::send \"")
                    + std::string(plugin_IPC::toYadoms::msg::descriptor()->full_name().data(),
                                  plugin_IPC::toYadoms::msg::descriptor()->full_name().size())
                    + "\", plugin API not ready to send message");

            if (!msg.IsInitialized())
                throw std::runtime_error(std::string("CApiImplementation::send \"")
                    + std::string(plugin_IPC::toYadoms::msg::descriptor()->full_name().data(),
                                  plugin_IPC::toYadoms::msg::descriptor()->full_name().size())
                    + "\", request is not fully initialized");
            ...
            if (!msg.SerializeWithCachedSizesToArray(serializedMessage.get()))
                throw std::runtime_error(std::string("CApiImplementation::send \"")
                    + std::string(plugin_IPC::toYadoms::msg::descriptor()->full_name().data(),
                                  plugin_IPC::toYadoms::msg::descriptor()->full_name().size())
                    + "\", fail to serialize request (too big ?)");
        ...
        case shared::event::kTimeout:
            {
                boost::lock_guard<boost::recursive_mutex> lock(m_onReceiveHookMutex);
                m_onReceiveHook.clear();
                throw std::runtime_error(std::string("No answer from Yadoms when sending message ")
                    + std::to_string(msg.OneOf_case()));
            }
        ...
        default:
            {
                boost::lock_guard<boost::recursive_mutex> lock(m_onReceiveHookMutex);
                m_onReceiveHook.clear();
                throw std::runtime_error(std::string("Invalid event received ")
                    + std::to_string(receivedEvtHandler.getEventId())
   ...
   void CApiImplementation::onReceive(const boost::shared_ptr<const unsigned char[]>& message,
                                       size_t messageSize)
    {
      ...
      if (!toPluginProtoBuffer.ParseFromArray(message.get(), static_cast<int>(messageSize)))
            throw shared::exception::CInvalidParameter(
                std::string("message : fail to parse received data into protobuf format (received buffer size is ")
        ...
        if (!m_initialized)
        {
            if (toPluginProtoBuffer.has_init())
                ...
                throw shared::exception::CInvalidParameter(std::string("Unexpected message ")
                    + std::to_string(toPluginProtoBuffer.OneOf_case())
                    + " when initialization");
            return;
        }
      ...
      switch (toPluginProtoBuffer.OneOf_case()) // NOLINT(clang-diagnostic-switch-enum)
        ...
        case plugin_IPC::toPlugin::msg::kDeviceRemoved: processDeviceRemoved(toPluginProtoBuffer.deviceremoved());
            break;
        default:
            throw shared::exception::CInvalidParameter(std::string("message : unknown message type ")
                + std::to_string(toPluginProtoBuffer.OneOf_case()));
        }
    }
                                            ...
                                            const std::map<std::string, std::string>& customMessageDataParams)
    {
      ...
      const auto request = req.mutable_pluginstate();
        ...
        case shared::plugin::yPluginApi::historization::EPluginState::kWaitDebuggerValue: request->set_pluginstate(
            ...
            break;
        default:
            throw std::out_of_range(std::string("CApiImplementation::setPluginState, unknown state ")
                + std::to_string(state));
        }
    ...
    void CApiImplementation::updateDeviceType(const std::string& device,
                                               ...
                                               const std::map<std::string, std::string>& customMessageDataParams) const
    {
      ...
      const auto request = req.mutable_devicestate();
        ...
        case shared::plugin::yPluginApi::historization::EDeviceState::kCustomValue: request->set_devicestate(
            ...
            break;
        default:
            throw std::out_of_range(std::string("CApiImplementation::updateDeviceState, unknown state ")
                + std::to_string(state));
        }
        request->set_custommessageid(customMessageId);
        request->set_device(device);
