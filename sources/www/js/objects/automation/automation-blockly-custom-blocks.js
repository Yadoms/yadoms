...
Blockly.Python["yadoms_keyword_value"] = function (block) {
    ...
    var readCommand = "yApi.readKeyword(" + dropdownKeyword + ")";

    if (Blockly.Yadoms.CurrentGenerationBlockIsTimed_ === true && Blockly.Yadoms.CurrentGenerationBlockTime_ != undefined) {
        readCommand = "yApi.readKeywordAt(" + dropdownKeyword + ", \"" + Blockly.Yadoms.CurrentGenerationBlockTime_.toISOString() + "\")";
    }

    var keyword = Blockly.Yadoms.data.keywords[dropdownKeyword];
    var code;
...
Blockly.Blocks["yadoms_wait_for_keywords"] = {
    ...
    appendKeywordSelectorStatement_: function (no) {
        ...
        Blockly.Yadoms.ConfigureBlockForYadomsKeywordSelection(this, false, ["numeric", "string", "bool", "nodata", "enum"], undefined, undefined, "deviceDd" + no, "keywordDd" + no, b)
...
Blockly.Python["yadoms_wait_for_keywords"] = function (block) {
    ...
    var i;
    var keyId;
    for (i = 0; i <= this.keywordMutatorCount_; i++) {
        keyId = block.getFieldValue("keywordDd" + i);
        kwList += keyId + " ,";
    }
    ...
    for (i = 0; i <= this.keywordMutatorCount_; i++) {
        //get the keywordId
        keyId = block.getFieldValue("keywordDd" + i); //construct the argument (= if/elif condition)
...
Blockly.Python["yadoms_notification_simple"] = function (block) {
    ...
    var value = Blockly.Python.valueToCode(block, block.inputValueName, Blockly.Python.ORDER_RELATIONAL) || "0";
    return "yApi.sendNotification(" + dropdownKeyword + "," + recipientId + ", \"" + value + "\")\n";
};
