...
namespace message
{
   ...
   std::string C4BSMessage::dump() const
   {
      std::stringstream ss;
      ss << std::setfill('0') << std::setw(2) << std::hex;
      ss << m_db3 << " " << m_db2 << " " << m_db1 << " " << m_db0;
      return ss.str();
   }
   ...
   bool C4BSMessage::isTeachIn() const
   {
      return (m_db0 & 0x08) != 0;//TODO constante à récupérer de l'XML
   }

   bool C4BSMessage::isEepProvided() const
   {
      return (m_db0 & 0x80) != 0;//TODO constante à récupérer de l'XML
   }
   
   unsigned int C4BSMessage::manufacturerId() const
   {
      return (m_db2 & 7) << 8 | m_db1;//TODO constante à récupérer de l'XML
   }

   unsigned int C4BSMessage::type() const
   {
      return (m_db3 & 3) << 5 | m_db2 >> 3;//TODO constante à récupérer de l'XML
   }

   unsigned int C4BSMessage::func() const
   {
      return m_db3 >> 2;//TODO constante à récupérer de l'XML
   }
} // namespace message
