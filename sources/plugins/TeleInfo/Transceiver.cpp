...
bool CTransceiver::isCheckSumOk(const unsigned char* buffer)
{
   ...
   for (i = 0; i < int(strlen(reinterpret_cast<const char*>(buffer))) - 2; i++)
   {
      checksum += buffer[i];
   }
   checksum = (checksum & 0x3F) + 0x20;
   return (checksum == buffer[strlen(reinterpret_cast<const char*>(buffer)) - 1]);
}
...
void CTransceiver::MatchLine(const unsigned char* buffer)
{
   ...
   auto pos = reinterpret_cast<const unsigned char *>(strchr(reinterpret_cast<const char*>(buffer), ' '));
   if (pos == nullptr)
      return;
   auto position = int(pos - buffer);
   char id[15] = "";
   strncpy(id, reinterpret_cast<const char*>(buffer), position);
   id[position] = 0;
   ...
   if (it != EEnumValuesTypes.end())
   {
      ...
      pos = reinterpret_cast<const unsigned char *>(strchr(reinterpret_cast<const char*>(buffer), ' '));
      if (pos == nullptr)
         return;
