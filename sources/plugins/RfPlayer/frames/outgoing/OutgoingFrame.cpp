...
namespace frames
{
   namespace outgoing
   {
      ...
      const std::string COutgoingFrame::generateAsciiCommand(boost::shared_ptr<shared::plugin::yPluginApi::IYPluginApi> api,
                                                             boost::shared_ptr<const shared::plugin::yPluginApi::IDeviceCommand> command)
      {
         ...
         else
         {
            ...
            {
               if ((details->exists("identifier.activeSection") && details->get<std::string>("identifier.activeSection") == "id32")
                  || (details->exists("identifier.content.id32.radio") && details->get<bool>("identifier.content.id32.radio")))
               {
               ...
               {
                  std::string addr = (boost::format("%1%%2%") % config->get<std::string>("identifier.content.idx10form.content.groupCode") % config->
                     get<int>("identifier.content.idx10form.content.unitCode")).str();
