...
namespace rfxcomMessages
{
		...
		m_keywords({ m_current1 , m_current2, m_current3, m_instantPower , m_batteryLevel, m_signalPower })
	{
		...
		{
			std::int64_t totalPower = rbuf.CURRENT_ENERGY.total3 << 24 | rbuf.CURRENT_ENERGY.total4 << 16 | rbuf.CURRENT_ENERGY.total5 << 8 | rbuf.
			...
			m_totalPower->set(static_cast<std::int64_t>(totalPower / 223.666));
		}
