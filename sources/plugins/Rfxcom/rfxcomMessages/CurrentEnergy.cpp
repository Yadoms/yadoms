...
boost::shared_ptr<std::queue<const shared::communication::CByteBuffer> > CCurrentEnergy::encode(boost::shared_ptr<ISequenceNumberProvider> seqNumberProvider) const
{
   BOOST_ASSERT_MSG(false, "CurrentEnergy is a read-only message, can not be encoded");
