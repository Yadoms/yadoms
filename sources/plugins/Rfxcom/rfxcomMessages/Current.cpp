...
CCurrent::CCurrent(boost::shared_ptr<yApi::IYPluginApi> api, const RBUF& rbuf, size_t rbufSize, boost::shared_ptr<const ISequenceNumberProvider> seqNumberProvider)
...
void CCurrent::Init(boost::shared_ptr<yApi::IYPluginApi> api)
{
   ...
   if (!api->deviceExists(m_deviceName))
   {
      ...
      api->declareKeyword(m_deviceName, m_current1);
      api->declareKeyword(m_deviceName, m_current2);
      api->declareKeyword(m_deviceName, m_current3);
...
{
   api->historizeData(m_deviceName, m_current1);
   api->historizeData(m_deviceName, m_current2);
   api->historizeData(m_deviceName, m_current3);
