#include "stdafx.h"
#include "ReadLearnedClientsCommand.h"
...
CReadLearnedClientsCommand::CReadLearnedClientsCommand(const boost::shared_ptr<IMessageHandler>& messageHandler)
    : m_messageHandler(messageHandler)
{
}

void CReadLearnedClientsCommand::sendAndReceive()
{
    YADOMS_LOG(information) << "Read Smart Ack learned clients...";

    CCommandSendMessage sendMessage(CCommandSendMessage::SA_RD_LEARNEDCLIENTS);
...
std::vector<boost::shared_ptr<CClient>> CReadLearnedClientsCommand::clients() const
{
    return m_clients;
}

void CReadLearnedClientsCommand::processAnswer(const response::CReceivedMessage& response,
                                               const std::string& requestName)
{
    ...
    std::vector<boost::shared_ptr<CClient>> clients(response.responseData().size() / clientSize);
    for (auto client = 0u; client < m_clients.size(); ++client)
    {
        m_clients.emplace_back(std::make_shared<CClient>(
...
std::vector<boost::shared_ptr<CClient>> CReadLearnedClientsCommand::unSerializeClients(
    const std::vector<unsigned char>& responseData)
{
    ...
    std::vector<boost::shared_ptr<CClient>> clients(responseData.size() / clientSize);
    for (auto client = 0u; client < m_clients.size(); ++client)
    {
        m_clients.emplace_back(std::make_shared<CClient>(
