...
COpenZWaveNode::COpenZWaveNode(const uint32 homeId,
                               const uint8 nodeId)
   : m_homeId(homeId),
     m_nodeId(nodeId)
{
...
void COpenZWaveNode::registerKeyword(OpenZWave::ValueID& value,
                                     bool includeSystemKeywords)
{
   ...
   if (m_keywords.find(keyword) == m_keywords.end())
      m_keywords[keyword] = COpenZWaveNodeKeywordFactory::createKeyword(value,
                                                                        m_homeId,
                                                                        m_nodeId,
                                                                        includeSystemKeywords);
}

boost::shared_ptr<IOpenZWaveNodeKeyword> COpenZWaveNode::getKeyword(OpenZWave::ValueID& value,
                                                                    bool includeSystemKeywords)
{
   auto keyword = COpenZWaveHelpers::GenerateKeywordName(value);
   if (m_keywords.find(keyword) == m_keywords.end())
   {
...
boost::shared_ptr<shared::plugin::yPluginApi::historization::IHistorizable> COpenZWaveNode::updateKeywordValue(OpenZWave::ValueID& value,
                                                                                                               bool includeSystemKeywords)
{
   boost::shared_ptr<IOpenZWaveNodeKeyword> kw = getKeyword(value,
                                                            includeSystemKeywords);
   kw->updateValue(value);
   return kw->getLastKeywordValue();
}

bool COpenZWaveNode::sendCommand(const std::string& keyword,
...
const bool COpenZWaveNode::match(const uint32 homeId,
                                 const uint8 nodeId)
{
