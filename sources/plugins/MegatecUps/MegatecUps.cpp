		...
		case yApi::IYPluginApi::kEventExtraQuery:
			{
				...
				if (extraQuery->getData()->query() == "resetBatteryDeadCommand")
				{
				...
				{
					YADOMS_LOG(error) << "Unknown extra command received : " << extraQuery->getData()->query();
					extraQuery->sendError("Unknown extra command received : " + extraQuery->getData()->query());
				}
