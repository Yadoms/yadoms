...
namespace equipments
{
											...
											bool forceHistorization)
	{
		...
		if (type == "C") updateIOFromDevice<specificHistorizers::CCounter, std::int64_t>(api, values, m_countersList, keywordsToHistorize, forceHistorization);
	...
	void CIPX800Equipment::historizePendingCommand(boost::shared_ptr<yApi::IYPluginApi> api, boost::shared_ptr<const yApi::IDeviceCommand> command)
	{
		...
		{
			std::int64_t newValue = boost::lexical_cast<std::int64_t>(command->getBody());
