		...
		try {
			...
			for (auto counter = 0; counter < m_WESIOMapping.clampQty; ++counter) {
				try {
					m_ClampList[counter]->updateFromDevice(api,
														   keywordsToHistorize,
														   results->get<std::int64_t>("WHPC" + boost::lexical_cast<std::string>(counter + 1) + "_val"));
				}
				catch (std::exception& e) {
			...
			for (auto counter = 0; counter < m_WESIOMapping.ticQty; ++counter) {
				...
				for (auto index = 0; index < CTIC::TICCountersNb; ++index) {
					counters.push_back(results->get<std::int64_t>("CPT" + boost::lexical_cast<std::string>(counter + 1) + "_INDEX_" + boost::lexical_cast<std::string>(index + 1)));
				}
