...
CIndicatorQualifier::CIndicatorQualifier(boost::shared_ptr<database::IPluginEventLoggerRequester> pluginLogger,
   ...
   :m_basicQualifier(pluginLogger, mainLogger), m_pluginLogger(pluginLogger)
{
}
...
void CIndicatorQualifier::signalCrash(const boost::shared_ptr<const shared::plugin::information::IInformation> pluginInformation, const std::string& reason)
{
...
int CIndicatorQualifier::computeQuality(const CIdentityForQualifier& identity) const
{
   ...
   std::vector<boost::shared_ptr<database::entities::CPluginEventLogger> > pluginEvents = m_pluginLogger->getPluginEvents(identity.getType(), identity.getVersion(), identity.getReleaseType(), boost::posix_time::ptime(fromDate));
