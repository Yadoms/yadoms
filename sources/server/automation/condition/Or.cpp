...
void COr::registerToNotificationCenter(boost::shared_ptr<INotificationObserverForRulesManager> notificationObserver, boost::shared_ptr<IConditionRootUpdater> conditionRootNotifier)
{
   for (Operands::const_iterator it = m_operands.begin(); it != m_operands.end(); ++it)
      (*it)->registerToNotificationCenter(notificationObserver, conditionRootNotifier);
}

void COr::unregisterFromNotificationCenter(boost::shared_ptr<INotificationObserverForRulesManager> notificationObserver)
{
   for (Operands::const_iterator it = m_operands.begin(); it != m_operands.end(); ++it)
