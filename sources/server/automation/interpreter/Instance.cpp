	...
	namespace interpreter
	{
      CInstance::CInstance(boost::shared_ptr<const shared::script::yInterpreterApi::IInformation> interpreterInformation,
		...
		std::string CInstance::loadScriptContent(const std::string& scriptPath) const
		{
			...
			{
				const auto errorMessage = std::string("Error when loading script content from interpreter ")
					+ m_interpreterInformation->getName()
					+ " : "
					+ e.what();
										  ...
										  const std::string& scriptContent) const
		{
         ...
         const auto request(boost::make_shared<CSaveScriptContentRequest>(scriptPath,
			...
			{
				request->sendError(std::string("Error when saving script content from interpreter ")
								   + m_interpreterInformation->getName()
								   + " : "
								   + e.what());
			}
		...
		bool CInstance::getAvailability() const
		{
         ...
         const auto request(boost::make_shared<CAvalaibleRequest>(callback));
			...
			{
				request->sendError(std::string("Error when requesting AvailableRequest on interpreter ")
								   + m_interpreterInformation->getName()
								   + " : "
								   + e.what());
			}
