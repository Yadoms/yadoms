	...
	namespace script
	{
							   ...
							   const boost::posix_time::time_duration& timeout)
		{
			...
			if (receivedEvtHandler.waitForEvents(timeout) == shared::event::kTimeout)
			{
				...
				throw std::runtime_error(std::string("No answer from script when sending message ") + std::to_string(pbMsg.OneOf_case()));
			}
										 ...
										 size_t messageSize)
		{
			...
			case script_IPC::toYadoms::msg::kGetKeywordDeviceName: processGetKeywordDeviceName(toYadomsProtoBuffer.getkeyworddevicename());
				break;
			default:
				throw std::invalid_argument(std::string("message : unknown message type ") + std::to_string(toYadomsProtoBuffer.OneOf_case()));
			}
		}
