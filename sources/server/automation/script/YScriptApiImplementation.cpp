	...
	namespace script
	{
		...
		void CYScriptApiImplementation::assertExistingKeyword(int keywordId) const
		{
			if (!m_keywordAccessLayer->keywordExists(keywordId))
				throw std::out_of_range(std::string("Keyword ID#") + std::to_string(keywordId) + " not found");
		}

		void CYScriptApiImplementation::assertExistingRecipient(int recipientId) const
		{
			if (!m_dbRecipientRequester->exists(recipientId))
				throw std::out_of_range(std::string("Recipient ID#") + std::to_string(recipientId) + " not found");
		}
		...
		int CYScriptApiImplementation::getKeywordId(const std::string& deviceName, const std::string& keywordName) const
		{
			...
			{
				throw std::out_of_range(std::string("getKeywordId, keyword ")
										+ keywordName
										+ " for device "
										+ deviceName
										+ " not found because device was not found");
			}
			...
			throw std::out_of_range(std::string("getKeywordId, several keywords found for ")
									+ keywordName
									+ " for device "
									+ deviceName
									+ ". Please rename keywords or device.");
		}
		...
		int CYScriptApiImplementation::getRecipientId(const std::string& firstName, const std::string& lastName) const
		{
			...
			{
				throw std::out_of_range(std::string("getRecipientId, recipient for ")
										+ firstName
										+ " "
										+ lastName
										+ " not found");
																								...
																								const std::string& timeout) const
		{
			...
			try
			{
				...
				default:
				{
					...
					throw std::out_of_range(std::string("CYScriptApiImplementation::waitForEvent : unknown result code ") + std::to_string(resultCode));
				}
				}
		...
		std::string CYScriptApiImplementation::getInfo(EInfoKeys key) const
		{
			...
			{
				throw std::out_of_range(std::string("getInfo(")
										+ std::to_string(key)
										+ "), error "
										+ e.what());
			}
		...
		std::string CYScriptApiImplementation::getKeywordName(int keywordId) const
		{
			...
			throw std::out_of_range(std::string("getKeywordName(")
		...
		std::string CYScriptApiImplementation::getKeywordDeviceName(int keywordId) const
		{
			...
			if (keyword)
			{
				...
				throw std::out_of_range(std::string("getKeywordDeviceName(")
										+ std::to_string(keywordId)
