...
#include "database/common/adapters/HugeDataVectorForKeywordDataAdapterV1.h"
#include "database/common/adapters/HugeDataVectorForKeywordSummaryDataAdapterV1.h"
#include "database/common/adapters/SingleValueAdapter.hpp"
#include "database/common/adapters/MultipleValueAdapter.hpp"
#include "database/DatabaseException.hpp"
#include "Keyword.h"
#include "database/common/adapters/HugeDataVectorForKeywordDataAdapterV2.h"
#include "database/common/adapters/HugeDataVectorForKeywordSummaryDataAdapterV2.h"

using namespace database::common::requesters;
...
boost::shared_ptr<database::entities::CAcquisition> CAcquisition::saveData(int keywordId,
                                                                           const std::string& data,
                                                                           boost::posix_time::ptime& dataTime)
{
   const auto keywordEntity = m_keywordRequester->getKeyword(keywordId);
   if (keywordEntity)
   {
...
boost::shared_ptr<database::entities::CAcquisition> CAcquisition::incrementData(int keywordId,
                                                                                ...
                                                                                boost::posix_time::ptime& dataTime)
{
   ...
   qLastKeywordValue->Select(
                         qLastKeywordValue->castNumeric(CKeywordTable::getLastAcquisitionValueColumnName())).
                      From(CKeywordTable::getTableName()).
                      Where(CKeywordTable::getIdColumnName(), CQUERY_OP_EQUAL, keywordId);

   // Update last value in Keyword table
   const auto qUpdateValue = m_databaseRequester->newQuery();
   qUpdateValue->Update(CKeywordTable::getTableName())
               .Set(CKeywordTable::getLastAcquisitionDateColumnName(), dataTime,
                    ...
                    qUpdateValue->math(qUpdateValue->coalesce(*qLastKeywordValue, 0), CQUERY_OP_PLUS,
                                       increment))
               .Where(CKeywordTable::getIdColumnName(), CQUERY_OP_EQUAL, keywordId);
...
boost::shared_ptr<database::entities::CAcquisition> CAcquisition::getAcquisitionByKeywordAndDate(
   int keywordId,
                                                         ...
                                                         int nbTotalLines)> exportOneLineFunction)
{
   ...
   const auto totalLines = adapter.getResults().size();
...
std::vector<boost::tuple<boost::posix_time::ptime, std::string>> CAcquisition::getKeywordData(
   ...
   int limit)
{
   const auto query = requestKeywordData(keywordId,
                                         timeFrom,
                                         timeTo,
                                         limit);

   adapters::CMultipleValueAdapter<boost::posix_time::ptime, std::string> adapter;
   ...
   boost::posix_time::ptime timeTo) const
{
   ...
   qSelect->OrderBy(CAcquisitionSummaryTable::getDateColumnName(), CQuery::kDesc);
...
std::string CAcquisition::getHugeVectorKeywordData(int keywordId,
                                                   boost::posix_time::ptime timeFrom,
                                                   boost::posix_time::ptime timeTo,
                                                   int limit)
{
   ...
   adapters::CHugeDataVectorForKeywordDataAdapterV1 adapter;
   m_databaseRequester->queryEntities(&adapter, *query);

   return adapter.getRawResults();
}

std::string CAcquisition::getHugeVectorKeywordDataV2(const std::set<int>& keywordIds,
                                                     const boost::posix_time::ptime& fromDate,
                                                     const boost::posix_time::ptime& toDate,
                                                     int limit)
{
   std::string str = R"({"acquisitions":[)";
   bool first = true;
   for (const auto& keywordId : keywordIds)
   {
      ...
      str += R"({"keywordId":)" + std::to_string(keywordId) + R"(, "acquisitions":)";
      ...
      adapters::CHugeDataVectorForKeywordDataAdapterV2 adapter;
      m_databaseRequester->queryEntities(&adapter, *query);
      str += adapter.getRawResults() + "}";
   }
   str += "]}";
...
std::string CAcquisition::getHugeVectorKeywordDataByHour(int keywordId,
                                                         ...
                                                         boost::posix_time::ptime timeTo)
{
   return getHugeVectorKeywordSummaryDataByType(entities::EAcquisitionSummaryType::kHour,
                                                keywordId,
                                                timeFrom,
                                                timeTo);
}
...
std::string CAcquisition::getHugeVectorKeywordDataByMonth(int keywordId,
                                                          ...
                                                          boost::posix_time::ptime timeTo)
{
   return getHugeVectorKeywordSummaryDataByType(entities::EAcquisitionSummaryType::kMonth,
                                                keywordId,
                                                timeFrom,
                                                timeTo);
}
...
std::string CAcquisition::getHugeVectorKeywordSummaryDataV2(const entities::EAcquisitionSummaryType& type,
                                                            ...
                                                            bool withCount)
{
   ...
   for (const auto& keywordId : keywordIds)
   {
      ...
      const auto query = getHugeVectorKeywordSummaryDataV2(type,
                                                           keywordId,
                                                           fromDate,
                                                           toDate,
                                                           limit,
                                                                ...
                                                                boost::posix_time::ptime timeTo) const
{
   ...
   qSelect->OrderBy(CAcquisitionSummaryTable::getDateColumnName(), CQuery::kDesc);
   adapters::CHugeDataVectorForKeywordSummaryDataAdapterV1 adapter;
   m_databaseRequester->queryEntities(&adapter, *qSelect);

   return adapter.getRawResults();
}

boost::shared_ptr<database::common::CQuery> CAcquisition::getHugeVectorKeywordSummaryDataV2(const entities::EAcquisitionSummaryType& type,
                                                                                            int keywordId,
                                                                                            const boost::posix_time::ptime& fromDate,
                                                                                            const boost::posix_time::ptime& toDate,
                                                                                            int limit,
                                                                                            ...
                                                                                            bool withCount) const
{
   ...
   std::vector<CDatabaseColumn> tables;
   tables.push_back(CAcquisitionSummaryTable::getDateColumnName());
   if (withAverage)
      tables.push_back(CAcquisitionSummaryTable::getAvgColumnName());
   if (withMin)
      tables.push_back(CAcquisitionSummaryTable::getMinColumnName());
   if (withMax)
      tables.push_back(CAcquisitionSummaryTable::getMaxColumnName());
   if (withCount)
      tables.push_back(CAcquisitionSummaryTable::getCountColumnName());

   qSelect->Select(tables).
            From(CAcquisitionSummaryTable::getTableName()).
            Where(CAcquisitionSummaryTable::getTypeColumnName(), CQUERY_OP_EQUAL, type).
            And(CAcquisitionSummaryTable::getKeywordIdColumnName(), CQUERY_OP_EQUAL, keywordId);

   if (!fromDate.is_not_a_date_time())
      qSelect->And(CAcquisitionSummaryTable::getDateColumnName(), CQUERY_OP_SUP_EQUAL, fromDate);
   if (!toDate.is_not_a_date_time())
      qSelect->And(CAcquisitionSummaryTable::getDateColumnName(), CQUERY_OP_INF_EQUAL, toDate);
                                         ...
                                         std::vector<int>& results)
{
   ...
   adapters::CSingleValueAdapterWithContainer intVectorAdapter(results);
   m_databaseRequester->queryEntities(&intVectorAdapter, *q);
}


boost::shared_ptr<database::entities::CAcquisitionSummary> CAcquisition::saveSummaryData(int keywordId,
                                                                                         entities::
                                                                                         EAcquisitionSummaryType curType,
                                                                                         ...
                                                                                         dataTime)
{
   /*
         ...
         */
   const auto keyword = m_keywordRequester->getKeyword(keywordId);
   ...
   if (keyword)
   {
      if (keyword->Type() == shared::plugin::yPluginApi::EKeywordDataType::kNumeric)
      {
         ...
         const auto ptTm = to_tm(dataTime);
         ...
         if (curType == entities::EAcquisitionSummaryType::kHour)
         {
            ...
            toDate = boost::posix_time::ptime(dataTime.date(),
                                              boost::posix_time::hours(ptTm.tm_hour) + boost::posix_time::
                                              minutes(59) + boost::posix_time::
                                              seconds(59));
            ...
            if (m_databaseRequester->supportInsertOrUpdateStatement())
            {
               //process the request
               const auto q = m_databaseRequester->newQuery();
               q->InsertOrReplaceInto(CAcquisitionSummaryTable::getTableName(),
                                      ...
                                      CAcquisitionSummaryTable::getCountColumnName()).
                  ...
                  Where(q->fromSubquery("acq", CAcquisitionTable::getKeywordIdColumnName()), CQUERY_OP_EQUAL,
                        keywordId).
                  And(q->fromSubquery("acq", CAcquisitionTable::getDateColumnName()), CQUERY_OP_SUP_EQUAL,
                      fromDate).
                  And(q->fromSubquery("acq", CAcquisitionTable::getDateColumnName()), CQUERY_OP_INF_EQUAL,
                      toDate);
               if (m_databaseRequester->queryStatement(*q) <= 0)
                  throw shared::exception::CEmptyResult(
                     "Fail to insert summary value (insert or update) " + curType.toString() + " data");
            }
            else
            {
               //update
               const auto compute = m_databaseRequester->newQuery();

               compute->Select(compute->as(compute->averageWithCast(CAcquisitionTable::getValueColumnName()),
                                           "avg"),
                               compute->as(compute->minWithCast(CAcquisitionTable::getValueColumnName()),
                                           "min"),
                               compute->as(compute->maxWithCast(CAcquisitionTable::getValueColumnName()),
                                           "max"),
               ...
               if (m_databaseRequester->queryStatement(*q) <= 0)
               {
                  ...
                  //insert
                  q->Clear();
                  q->InsertInto(CAcquisitionSummaryTable::getTableName(),
                                CAcquisitionSummaryTable::getTypeColumnName(),
                                CAcquisitionSummaryTable::getDateColumnName(),
                                CAcquisitionSummaryTable::getKeywordIdColumnName(),
                                CAcquisitionSummaryTable::getAvgColumnName(),
                     ...
                     Select(curType, fromDate, keywordId,
                            q->averageWithCast(
                               q->fromSubquery("acq", CAcquisitionTable::getValueColumnName())),
                            q->minWithCast(q->fromSubquery("acq", CAcquisitionTable::getValueColumnName())),
                            q->maxWithCast(q->fromSubquery("acq", CAcquisitionTable::getValueColumnName())),
                            q->count()).
                     From(q->as(CAcquisitionTable::getTableName(), "acq")).
                     Where(q->fromSubquery("acq", CAcquisitionTable::getKeywordIdColumnName()),
                           CQUERY_OP_EQUAL, keywordId).
                     And(q->fromSubquery("acq", CAcquisitionTable::getDateColumnName()), CQUERY_OP_SUP_EQUAL,
                         fromDate).
                     And(q->fromSubquery("acq", CAcquisitionTable::getDateColumnName()), CQUERY_OP_INF_EQUAL,
                         toDate);
                  ...
                  if (m_databaseRequester->queryStatement(*q) <= 0)
                     throw shared::exception::CEmptyResult(
                        "Fail to insert summary value " + curType.toString() + " data");
               }
            }
         }
         else
         {
            ...
            if (m_databaseRequester->supportInsertOrUpdateStatement())
            {
               ...
               q->InsertOrReplaceInto(CAcquisitionSummaryTable::getTableName(),
                  ...
                  Select(curType, fromDate, keywordId,
                         q->math(q->sum(q->math(
                                    q->fromSubquery("acq", CAcquisitionSummaryTable::getAvgColumnName()),
                                    CQUERY_OP_MUL,
                                    q->fromSubquery("acq", CAcquisitionSummaryTable::getCountColumnName()))),
                                 CQUERY_OP_DIVIDE,
                                 q->sum(q->fromSubquery(
                                    "acq", CAcquisitionSummaryTable::getCountColumnName()))),
                  ...
                  Where(q->fromSubquery("acq", CAcquisitionSummaryTable::getKeywordIdColumnName()),
                        CQUERY_OP_EQUAL, keywordId).
                  And(q->fromSubquery("acq", CAcquisitionSummaryTable::getTypeColumnName()), CQUERY_OP_EQUAL,
                      toQuery.toString()).
                  And(q->fromSubquery("acq", CAcquisitionSummaryTable::getDateColumnName()),
                      CQUERY_OP_SUP_EQUAL, fromDate).
                  And(q->fromSubquery("acq", CAcquisitionSummaryTable::getDateColumnName()),
                      CQUERY_OP_INF_EQUAL, toDate);
            ...
            else
            {
               ...
               compute->Select(compute->as(compute->math(
                                              compute->sum(compute->math(
                                                 CAcquisitionSummaryTable::getAvgColumnName(), CQUERY_OP_MUL,
                                                 CAcquisitionSummaryTable::getCountColumnName())),
                                              CQUERY_OP_DIVIDE,
                                              compute->sum(CAcquisitionSummaryTable::getCountColumnName())),
                                           "avg"),
                               ...
                               compute->as(compute->sum(CAcquisitionSummaryTable::getCountColumnName()),
                                           "count")).
                        From(CAcquisitionSummaryTable::getTableName()).
                        Where(CAcquisitionSummaryTable::getKeywordIdColumnName(), CQUERY_OP_EQUAL, keywordId).
                        And(CAcquisitionSummaryTable::getTypeColumnName(), CQUERY_OP_EQUAL,
                            toQuery.toString()).
               ...
               if (m_databaseRequester->queryStatement(*q) <= 0)
               {
                  ...
                  q->InsertInto(CAcquisitionSummaryTable::getTableName(),
                     ...
                     Where(q->fromSubquery("acq", CAcquisitionSummaryTable::getKeywordIdColumnName()),
                           CQUERY_OP_EQUAL, keywordId).
                     And(q->fromSubquery("acq", CAcquisitionSummaryTable::getTypeColumnName()),
                         CQUERY_OP_EQUAL, toQuery.toString()).
                     And(q->fromSubquery("acq", CAcquisitionSummaryTable::getDateColumnName()),
                         CQUERY_OP_SUP_EQUAL, fromDate).
                     And(q->fromSubquery("acq", CAcquisitionSummaryTable::getDateColumnName()),
                         CQUERY_OP_INF_EQUAL, toDate);
                                     ...
                                     boost::posix_time::ptime& date)
{
   ...
   const auto ptTm = to_tm(date);
   if (curType == entities::EAcquisitionSummaryType::kHour)
   {
      fromDate = boost::posix_time::ptime(date.date(), boost::posix_time::hours(ptTm.tm_hour));
   }
   ...
   const auto checkQuery = m_databaseRequester->newQuery();
...
boost::shared_ptr<database::common::CQuery> CAcquisition::requestKeywordData(int keywordId,
                                                                             boost::posix_time::ptime timeFrom,
                                                                             boost::posix_time::ptime timeTo,
                                                                             int limit) const
{
   ...
   qSelect->OrderBy(CAcquisitionTable::getDateColumnName(), CQuery::kDesc);
