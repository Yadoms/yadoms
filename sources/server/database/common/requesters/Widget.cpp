...
CWidget::CWidget(const boost::shared_ptr<IDatabaseRequester>& databaseRequester)
...
int CWidget::addWidget(const entities::CWidget& newWidget)
{
   ...
   if (newWidget.Id() != 0)
   {
      qInsert->InsertInto(CWidgetTable::getTableName(), CWidgetTable::getIdColumnName(), CWidgetTable::getIdPageColumnName(),
                          CWidgetTable::getTypeColumnName(), CWidgetTable::getSizeXColumnName(), CWidgetTable::getSizeYColumnName(),
                          CWidgetTable::getPositionColumnName(), CWidgetTable::getTitleColumnName(), CWidgetTable::getConfigurationColumnName()).
               Values(newWidget.Id(), newWidget.IdPage(), newWidget.Type(), newWidget.SizeX(), newWidget.SizeY(), newWidget.Position(),
                      newWidget.Title(), newWidget.Configuration());
   }
   else
   {
      qInsert->InsertInto(CWidgetTable::getTableName(), CWidgetTable::getIdPageColumnName(), CWidgetTable::getTypeColumnName(),
                          CWidgetTable::getSizeXColumnName(), CWidgetTable::getSizeYColumnName(), CWidgetTable::getPositionColumnName(),
                          CWidgetTable::getTitleColumnName(), CWidgetTable::getConfigurationColumnName()).
               Values(newWidget.IdPage(), newWidget.Type(), newWidget.SizeX(), newWidget.SizeY(), newWidget.Position(), newWidget.Title(),
                      newWidget.Configuration());
   }
...
boost::shared_ptr<database::entities::CWidget> CWidget::getWidget(int widgetId)
{
   ...
   throw shared::exception::CEmptyResult(std::string("Cannot retrieve Widget Id=")
...
void CWidget::updateWidgetConfiguration(int widgetId,
                                        const std::string& newConfiguration)
{
...
void CWidget::updateWidgetSize(int widgetId,
                               int sizeX,
                               int sizeY)
{
   auto qUpdate = m_databaseRequester->newQuery();
...
void CWidget::updateWidgetPosition(int widgetId,
                                   int position) const
{
   auto qUpdate = m_databaseRequester->newQuery();
...
void CWidget::updateWidget(const entities::CWidget& widgetToUpdate,
