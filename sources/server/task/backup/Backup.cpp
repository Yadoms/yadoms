...
namespace task
{
   namespace backup
   {
      ...
      void CBackup::doWork(TaskProgressFunc functor)
      {
      ...
      boost::filesystem::path CBackup::prepare() const
      {
         auto tempFolder = boost::filesystem::temp_directory_path() / "yadomsBackup";

         if (!checkEnoughSpace(tempFolder.parent_path()))
         {
            YADOMS_LOG(warning) << "No enough space to backup into " << tempFolder;

            tempFolder = m_pathProvider->backupPath() / "yadomsBackup";
            YADOMS_LOG(warning) << "Retry in " << tempFolder << "...";
            if (!checkEnoughSpace(tempFolder.parent_path()))
               throw shared::exception::CException("No enough space in " + tempFolder.string() + " to process backup");
         }
      ...
      bool CBackup::backupFiles(const boost::filesystem::path& tempFolder)
      {
         //backup database (1 -> 40%)
         if (m_dataBackupInterface->backupSupported())
         {
            m_dataBackupInterface->backupData(tempFolder.string(), [&](int remaining, int total, std::string i18nMessage, std::string error)
            {
      ...
      boost::filesystem::path CBackup::makeZipArchive(const boost::filesystem::path& tempFolder)
      {
      ...
      void CBackup::onZipEDone(const void* pSender,
                               const Poco::Zip::ZipLocalFileHeader& hdr)
      {
         if (hdr.isFile())
         {
            ...
            onProgressionUpdatedInternal(m_fileCountToZip - m_currentFileCount,
                                         ...
                                         60.0f,
                                         98.0f,
                                         i18n::CClientStrings::BackupCompress);
            YADOMS_LOG(debug) << " ZIP : " << hdr.getFileName();
         }
      }

      void CBackup::cleanup(const boost::filesystem::path& tempFolder) const
      {
         onProgressionUpdatedInternal(0,
                                      100,
                                      98.0f,
                                      99.0f,
                                      i18n::CClientStrings::BackupClean);
