...
namespace task
{
   ...
   {
      const std::string CExportAcquisitions::TaskName = "exportAcquisitions";

      CExportAcquisitions::CExportAcquisitions(boost::shared_ptr<const IPathProvider> pathProvider,
      ...
      const std::string& CExportAcquisitions::getName() const
      {
         return TaskName;
      }

      void CExportAcquisitions::onSetTaskId(const std::string& taskId)
      {
      }

      void CExportAcquisitions::onProgressionUpdatedInternal(int remaining,
                                                             int total,
                                                             float currentPart,
                                                             float totalPart,
      ...
      void CExportAcquisitions::doWork(TaskProgressFunc functor)
      {
         m_reportRealProgress = functor;
         doWork(0);
      }

      bool CExportAcquisitions::isCancellable() const
      {
         return true;
      }

      void CExportAcquisitions::doWork(int currentTry)
      {
      ...
      bool CExportAcquisitions::checkEnoughSpace(const boost::filesystem::path& where) const
      {
         return true;
      }

      boost::filesystem::path CExportAcquisitions::prepare()
      {
         auto tempFolder = boost::filesystem::temp_directory_path() / "exportAcquisitions";

         if (!checkEnoughSpace(tempFolder.parent_path()))
         {
            YADOMS_LOG(warning) << "No enough space to extract data into " << tempFolder;

            tempFolder = m_pathProvider->backupPath() / "exportAcquisitions";
      ...
      void CExportAcquisitions::collectDataTo(const boost::filesystem::path& tempFolder)
      {
      ...
      boost::filesystem::path CExportAcquisitions::makeZipArchive(const boost::filesystem::path& tempFolder)
      {
         ...
         auto zipFilenameFinal = m_pathProvider->backupPath() / "exportAcquisitions.zip";
         const auto zipFilename = m_pathProvider->backupPath() / "exportAcquisitions.zip.inprogress";
         ...
         try
         {
            ...
            c.EDone += Poco::Delegate<CExportAcquisitions, const Poco::Zip::ZipLocalFileHeader>(this, &CExportAcquisitions::onZipEDone);
      ...
      void CExportAcquisitions::onZipEDone(const void* pSender,
      ...
      void CExportAcquisitions::cleanup(const boost::filesystem::path& tempFolder)
      {
