...
{
   if (m_thread)
      throw shared::exception::CException("CApplicationStopHandler::configure must not be called twice");
...
void CApplicationStopHandler::registerForAppEnds(boost::shared_ptr<shared::event::CEventHandler> & handler, const int code)
{
   m_appendHandler.push_back(std::make_pair(handler, code));
}

void CApplicationStopHandler::NotifyApplicationEnds()
{
   std::vector< std::pair< boost::shared_ptr<shared::event::CEventHandler>, int > >::iterator i;
   for (i = m_appendHandler.begin(); i != m_appendHandler.end(); ++i)
   {
      try
      {
         if (i->first)
