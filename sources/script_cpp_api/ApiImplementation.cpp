...
void CYScriptApiImplementation::sendRequest(const script_IPC::toYadoms::msg& request) const
{
    try
    {
        ...
        if (!m_sendMessageQueue || !m_messageCutter)
            throw std::runtime_error(std::string("CYScriptApiImplementation::send \"")
                + std::string(script_IPC::toYadoms::msg::descriptor()->full_name().data(),
                              script_IPC::toYadoms::msg::descriptor()->full_name().size())
                + "\", script API not ready to send message");
    ...
    {
        throw std::overflow_error(std::string("yScriptApiWrapper::sendRequest \"")
            + std::string(script_IPC::toYadoms::msg::descriptor()->full_name().data(), script_IPC::toYadoms::msg::descriptor()->full_name().size())
            + "\" : Error at IYScriptApi method call, "
            + ex.what());
    }
}
