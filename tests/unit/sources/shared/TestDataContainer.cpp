...
BOOST_AUTO_TEST_SUITE(TestDataContainer)
	...
	BOOST_AUTO_TEST_CASE(ContainerToVectorOfContainers)
	{
      ...
      {\
		...
		BOOST_CHECK_THROW(const auto out = inIsNotAnArray.get<std::vector<boost::shared_ptr<shared::CDataContainer>>>(),
		                  shared::exception::COutOfRange);
	}

	boost::shared_ptr<shared::CDataContainer> GenerateContainer(const shared::CDataContainer& data)
	{
		boost::shared_ptr<shared::CDataContainer> error = shared::CDataContainer::make();
		error->set("result", true);
		error->set("message", std::string());
		error->set("data", data);
		return error;
	}

	boost::shared_ptr<shared::CDataContainer> GenerateContainer(const boost::shared_ptr<shared::CDataContainer>& data)
	{
		boost::shared_ptr<shared::CDataContainer> error = shared::CDataContainer::make();
		error->set("result", true);
		error->set("message", std::string());
	...
	boost::shared_ptr<shared::serialization::IDataSerializable> Maketest(const unsigned int testcount)
	{
		...
		return GenerateContainer(result);
	}
