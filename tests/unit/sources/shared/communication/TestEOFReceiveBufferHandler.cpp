...
void pushThreaded(shared::communication::CEOFReceiveBufferHandler& buffer,
                  unsigned char sentTab[],
                  int nbPushs)
{
   for (auto iPush = 0; iPush < nbPushs; iPush++)
      buffer.push(shared::communication::CByteBuffer(sentTab, sizeof(sentTab)));
}

BOOST_AUTO_TEST_CASE(ThreadSafety)
{
   enum { EOF_CHAR = ' ' };
   unsigned char sentTab[] = { 'a', 'b', 'c', 'd', 'e', 'f', 'g', EOF_CHAR };

   shared::event::CEventHandler evtHandler;
   shared::communication::CEOFReceiveBufferHandler buffer(evtHandler, kEvtId, EOF_CHAR);

   // Flushing when pushing data
   boost::thread pushThread(pushThreaded, buffer, sentTab, 1000);
   for (auto iFlush = 0; iFlush < 1000; iFlush++)
      buffer.flush();

   // Check data : all received messages must be complete
   auto allreceived = false;
   auto receivedMessagesCount = 0;
   while (allreceived)
   {
      switch (evtHandler.waitForEvents(boost::posix_time::seconds(1)))
      {
      case kEvtId:
         {
            BOOST_CHECK_EQUAL(evtHandler.isEventType<const shared::communication::CByteBuffer>(), true);
            auto receivedBuffer = evtHandler.getEventData<const shared::communication::CByteBuffer>();
            BOOST_CHECK_EQUAL(buffersAreEqual(shared::communication::CByteBuffer(sentTab, sizeof(sentTab)), receivedBuffer), true);
      ...
      default:
         {
            BOOST_FAIL("Invalid event");
            break;
         }
      }
   }
   std::cout << " Number of received messages (not flushed) : " << receivedMessagesCount << std::endl;
   pushThread.join();
}
