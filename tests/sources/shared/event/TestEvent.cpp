...
//--------------------------------------------------------------
/// \brief	    Test for specific problem (crash in popEvent because pop with no event)
/// \result         No Error
//--------------------------------------------------------------
class CNotification
{
public:
   enum { kNotification = shared::event::kUserFirstId };
   CNotification(const std::string& s) : m_s(s) {}
   std::string m_s;
};

void CrashInPopEventThreadSender(shared::event::CEventHandler* target, const std::string& idThread)
{
   enum
   {
      kSendSensorsStateTimerEventId = shared::event::kUserFirstId
   };

   shared::event::CEventHandler evtHandler;
   evtHandler.createTimer(kSendSensorsStateTimerEventId, shared::event::CEventTimer::kPeriodic, boost::posix_time::seconds(10));
   ...
   while(count < 10)
   {
      ...
      case kSendSensorsStateTimerEventId:
         {
            std::cout << "postEvent " << idThread << std::endl;
            for(int i = 0 ; i<100;++i)
            {
               boost::shared_ptr< CNotification > notificationData(new CNotification(idThread));
               target->postEvent<boost::shared_ptr< CNotification > >(CNotification::kNotification, notificationData);
      ...
      default:
         {
            std::cout << "Unknown ou unsupported message id " << evtHandler.getEventId() << std::endl;
            break;
         }
      }
   }
...
BOOST_AUTO_TEST_CASE(CrashInPopEvent)
{
   shared::event::CEventHandler evtHandler;

   const unsigned int NbThreads = 10;
   boost::thread threads[NbThreads];
   for (unsigned int idThread = 0 ; idThread < NbThreads ; ++idThread)
      threads[idThread] = boost::thread(CrashInPopEventThreadSender, &evtHandler, "sender " + boost::lexical_cast<std::string>(idThread));

   bool threadsAreAlive = true;
   while(threadsAreAlive)
   {
      switch (evtHandler.waitForEvents(boost::posix_time::milliseconds(100)))
      {
      case shared::event::kTimeout:
         {
            threadsAreAlive = false;
            for (unsigned int idThread = 0 ; idThread < NbThreads && !threadsAreAlive ; ++idThread)
               if (!threads[idThread].timed_join(boost::posix_time::milliseconds(0)))
                  threadsAreAlive = true;
         }
         break;
      case CNotification::kNotification:
         {
            //check if notification is a newAcquisition
            if (evtHandler.isEventType< boost::shared_ptr<CNotification> >())
            {
               boost::shared_ptr<CNotification> notif = evtHandler.getEventData< boost::shared_ptr<CNotification> >();
               std::cout << "notification received : " << notif->m_s << std::endl;
            }
            break;
         }
   ...
   // No check here : the aim of this test is to reproduce a crash condition, so if no crash, no error...
}
