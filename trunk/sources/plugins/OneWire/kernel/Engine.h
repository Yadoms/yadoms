...
public:
   ...
   //--------------------------------------------------------------
   CEngine(boost::shared_ptr<yApi::IYPluginApi> context, boost::shared_ptr<const IConfiguration> configuration);
...
protected:
   ...
   //--------------------------------------------------------------
   void scanNetworkNode(const boost::filesystem::path& slavesFile, std::map<std::string, boost::shared_ptr<device::IDevice> >& devices) const;
   ...
   /// \return new IDevice instance
   //--------------------------------------------------------------
   boost::shared_ptr<device::IDevice> createDevice(EOneWireFamily family, const std::string& id, const boost::filesystem::path& devicePath) const;
