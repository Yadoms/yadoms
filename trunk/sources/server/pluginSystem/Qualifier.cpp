...
int CQualifier::computeQuality(const CIdentityForQualifier& identity) const
{
   ...
   std::vector<const boost::shared_ptr<database::entities::CPluginEventLogger> > pluginEvents = m_pluginLogger->getPluginEvents(identity.getName(), identity.getVersion(), identity.getReleaseType(), boost::posix_time::ptime(fromDate));
   for (std::vector<const boost::shared_ptr<database::entities::CPluginEventLogger> >::const_iterator it = pluginEvents.begin() ; it != pluginEvents.end() ; ++it)
   {
      switch((*it)->EventType())
      {
